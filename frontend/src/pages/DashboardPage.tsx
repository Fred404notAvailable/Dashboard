...
import { reportsApi, syncApi, exportApi, pdfApi, settingsApi, expensesApi } from '../api/client';
import {
...
export default function DashboardPage() {
  ...
  const handleOpenEditExpense = (item: ExpenseItem) => {
    ...
    setExpenseForm({
      ...
      expenseDate: item.expenseDate ? format(new Date(item.expenseDate), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd'),
  ...
  const handleSaveExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.error || 'Failed to save expense. Please check input values.');
    } finally {
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter((item) => {
      const matchSearch =
        ...
        (item.vendor && item.vendor.toLowerCase().includes(expenseSearch.toLowerCase())) ||
        (item.notes && item.notes.toLowerCase().includes(expenseSearch.toLowerCase()));

      const matchCategory =
        expenseCategoryFilter === '' || item.category === expenseCategoryFilter;

      const matchPayment =
        expensePaymentFilter === '' || item.paymentMethod === expensePaymentFilter;

      return matchSearch && matchCategory && matchPayment;
    });
  }, [expenses, expenseSearch, expenseCategoryFilter, expensePaymentFilter]);
  ...
  return (
    <div className="app-layout">
      ...
      <header className="app-header">
        <div className="app-header__brand">
          ...
          <div>
            ...
            <div className="app-header__subtitle">Registration & Event Management Dashboard</div>
          </div>
        </div>
        ...
        <div className="app-header__actions">
          ...
          {(user?.role === 'admin' || user?.role === 'overall') && (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '0.8rem', color: '#B0B0B0' }}>
              {user?.displayName} <span className="badge" style={{ fontSize: '0.7rem', textTransform: 'uppercase' }}>{user?.role}</span>
            </span>
      ...
      <main className="app-main">
        {/* Navigation Tabs (Registration Analytics vs Financials & Expenses) */}
        ...
        {(filters.registrationType || filters.department || filters.event) && (
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
            ...
            <button className="btn btn--ghost btn--sm" onClick={() => dispatch({ type: 'CLEAR_ALL' })}>
              Clear all
            </button>
          </div>
        )}
        ...
        {activeTab === 'analytics' && data && (
          <>
            ...
            {/* Charts Row 1: 4 interactive charts */}
            ...
            {/* Year-wise + School Comparison + Event Combos (3 Column Grid) */}
                    ...
                    <tbody>
                      {data.schoolComparison.map(s => (
                        <tr key={s.school}>
                          ...
                          <td style={{ color: GOLD }}>
                            {hasFinancialAccess && s.revenue !== null ? (
                              `₹${s.revenue.toLocaleString()}`
                            ) : (
                              <span style={{ color: '#888', fontSize: '0.75rem', display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
                                <FiLock /> Locked
                              </span>
                            )}
                          </td>
            ...
            {/* Goal Tracker + Data Quality + Registration Velocity (3 Column Grid) */}
            <div className="charts-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: '24px' }}>
              {/* Goal Tracker */}
              <div className="card animate-fade-in">
                <div className="section-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><FiTarget /> Registration Goal</span>
                  {(user?.role === 'admin' || user?.role === 'overall') && !editingGoal && (
                    <button
      ...
      <footer style={{
        ...
        <span>Registration & Event Analytics Dashboard v1.2</span>
      </footer>
          ...
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSaveExpense}>
              <div className="form-group" style={{ marginBottom: '16px' }}>
                <label className="form-group__label">Expense Title / Description *</label>
                <input
                  type="text"
                  className="form-group__input"
                  placeholder="e.g. Stage Sound & Lighting setup"
